...
def _get_submission_arxiv_id_or_none(submission):
    try:
        ...
        # support formats: arXiv:1605.03286 and arXiv:physics/0612096
...
def _soup_to_articles(soup, new_submission_date):
    ...
    latest_submission_date = _get_date_from_new_submissions_title_or_none(new_submissions_title)
    _assert_or_exit(latest_submission_date,
                    'Could not determine the latest submission date.')

    new_submissions = []
    crosslists = []
    replacements = []

    if latest_submission_date < new_submission_date:
        logging.warning('Ignoring articles because no new submissions detected for ({}).'.format(new_submission_date))
    else:
...
def _new_submission_date_or_exit():
    """Check the ARXIV_CS_FEED_URL to determine whether new articles have been published across all sites."""
...
def run():
    ...
    new_submission_date = _new_submission_date_or_exit()
    ...
    for feed_url in arxiv_feed_urls:
        ...
        articles = _soup_to_articles(soup, new_submission_date)
    ...
    for submission in new_submissions:
        ...
        try:
            Article.objects.create(id_arxiv=submission['id_arxiv'],
                                   ...
                                   date_submitted=new_submission_date,
                                   date_updated=new_submission_date,
                                   is_processed=False)
    ...
    for replacement in replacements:
        ...
        try:
            ...
            article.is_processed = False
            article.date_updated = new_submission_date
            article.save()
